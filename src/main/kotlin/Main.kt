        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        val commentsWithAuthor = getComments(client, post.id).map { comment ->
                            async {
                                CommentWithAuthor(getAuthor(client,comment.authorId), comment)
                            }
                        }.awaitAll()
                        async {
                            PostWithAuthor(
                                getAuthor(client, post.authorId),
                                PostWithComments(post,commentsWithAuthor)
                            )
                        }
                    }.awaitAll()
                println(posts)
